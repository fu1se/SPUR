...
`app` устанавливается на каждую машину, которая будет подключаться к
другим. `app-server` — только на одну машину с публичным IP
(rendezvous-сервер).
...
| `--db` | `~/.config/localizator/state.db` | файл состояния (SQLite) — зарегистрированные пиры, mesh-сети и их участники |
...
`app register` (см. ниже) — она не требует никакой предварительной
...
сохраняется в `~/.config/localizator/identity.key` — при последующих
...
сообщить друг другу вывод `app whoami` — сервер этого за них не делает.
...
```sh
app register --server SERVER:4443
```
...
```sh
app join-network --server SERVER:4443 --network home --invite <токен>
```
...
| `app whoami` | Свой peer-id, без обращения к сети |
...
| `app join` | Присоединиться к mesh-сети (реальный TUN + WireGuard, нужен root) |
| `app join-network` | Диагностика: та же координация сети, без TUN и без root |
...
| `app version` / `app-server version` | Версия сборки |
